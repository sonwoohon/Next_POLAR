  ...
  implements IContactReadStatusRepository
{
  ...
  async findByRoomAndReader(
    ...
    console.log(
      `[Repository] 읽음 상태 조회 시작: contactRoomId=${contactRoomId}, readerId=${readerId}`
    );
  ...
  async updateReadStatus(
    ...
    console.log(
      `[Repository] 읽음 상태 업데이트/생성 시작: contactRoomId=${contactRoomId}, readerId=${readerId}, lastReadMessageId=${lastReadMessageId}`
    );
    ...
    if (existingRecord) {
      ...
      console.log(
        `[Repository] 기존 last_read_message_id: ${existingRecord.lastReadMessageId}, 최대 메시지 ID: ${maxMessageId}`
      );
      ...
      if (maxMessageId > existingRecord.lastReadMessageId) {
        ...
        console.log(
          `[Repository] 최대 메시지 ID(${maxMessageId})가 기존(${existingRecord.lastReadMessageId})보다 작거나 같아 업데이트 건너뜀`
        );
