...
export class UserValidator {
  static validatePhoneNumber(phoneNumber: string): void {
    ...
    if (!/^\d+$/.test(cleanPhoneNumber)) {
      console.error(`[Validator] 전화번호 검증 실패: 숫자가 아닌 문자 포함 - ${phoneNumber}`);
      throw new ValidationError('전화번호는 숫자만 입력 가능합니다.');
    }
    if (cleanPhoneNumber.length < 10 || cleanPhoneNumber.length > 11) {
      console.error(`[Validator] 전화번호 검증 실패: 길이 오류 - ${cleanPhoneNumber.length}자리`);
  ...
  static validatePassword(password: string): void {
    ...
    if (!password || password.length < 6) {
      console.error(`[Validator] 비밀번호 검증 실패: 길이 부족 - ${password.length}자리`);
  ...
  static validateProfileImageUrl(url: string): void {
    ...
    if (url && !url.startsWith('http') && !url.startsWith('https')) {
      console.error(`[Validator] 프로필 이미지 URL 검증 실패: 프로토콜 오류 - ${url}`);
      throw new ValidationError('프로필 이미지 URL은 http 또는 https로 시작해야 합니다.');
    }
  ...
  static validateName(name: string): void {
    ...
    if (name.length > 50) {
      console.error(`[Validator] 이름 검증 실패: 길이 초과 - ${name.length}자리`);
...
export class CommonUserUseCase {
  ...
  async deleteProfileImage(id: number): Promise<CommonUserEntity> {
    ...
    try {
      ...
      if (!existingUser) {
        console.error(`[UseCase] 프로필 이미지 삭제 실패 - 사용자를 찾을 수 없음 - ID: ${id}`);
        throw new ValidationError('사용자를 찾을 수 없습니다.');
      }

      console.log(`[UseCase] 기존 사용자 정보 - ID: ${id}`, existingUser.toJSON());
      ...
      console.log(`[UseCase] 프로필 이미지 삭제용 Entity 생성 완료 - ID: ${id}`, updatedUser.toJSON());

      const result = await this.userRepository.updateUser(id, updatedUser);
      if (!result) {
        console.error(`[UseCase] 프로필 이미지 삭제 실패 - Repository 업데이트 실패 - ID: ${id}`);
        throw new ValidationError('프로필 이미지 삭제에 실패했습니다.');
      }

      console.log(`[UseCase] 프로필 이미지 삭제 성공 - ID: ${id}`, result.toJSON());
      return result;
    } catch (error) {
      console.error(`[UseCase] 프로필 이미지 삭제 중 오류 발생 - ID: ${id}`, error);
      throw error;
    }
  }

  // 사용자 프로필 업데이트 (비밀번호 포함)
  async updateUserProfile(id: number, updates: UserProfileUpdate): Promise<CommonUserEntity> {
    ...
    try {
      ...
      const existingUser = await this.userRepository.getUserById(id);
      if (!existingUser) {
        console.error(`[UseCase] 프로필 업데이트 실패 - 사용자를 찾을 수 없음 - ID: ${id}`);
        throw new ValidationError('사용자를 찾을 수 없습니다.');
      }
      ...
      if (updates.phoneNumber !== undefined) {
        console.log(`[UseCase] 전화번호 검증 - ID: ${id}, 값: ${updates.phoneNumber}`);
      ...
      if (updates.profileImgUrl !== undefined) {
        console.log(`[UseCase] 프로필 이미지 URL 검증 - ID: ${id}, 값: ${updates.profileImgUrl}`);
      ...
      if (updates.password !== undefined) {
        console.log(`[UseCase] 비밀번호 검증 - ID: ${id}, 길이: ${updates.password.length}`);
      ...
      console.log(`[UseCase] 업데이트용 Entity 생성 완료 - ID: ${id}`, updatedUser.toJSON());

      const result = await this.userRepository.updateUser(id, updatedUser);
      if (!result) {
        console.error(`[UseCase] 프로필 업데이트 실패 - Repository 업데이트 실패 - ID: ${id}`);
        throw new ValidationError('사용자 정보 업데이트에 실패했습니다.');
      }

      console.log(`[UseCase] 프로필 업데이트 성공 - ID: ${id}`, result.toJSON());
      return result;
    } catch (error) {
      console.error(`[UseCase] 프로필 업데이트 중 오류 발생 - ID: ${id}`, error);
      throw error;
    }
  }
} 