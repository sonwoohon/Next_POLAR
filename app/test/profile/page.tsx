...
export default function ProfilePage() {
  ...
  const handleImageUpload = async () => {
    ...
    try {
      ...
      const data = await response.json();
      console.log('[Client] API 응답:', data);

      if (response.ok && data.url) {
        console.log('[Client] 이미지 업로드 성공, 사용자 정보 조회 시작');
        // API에서 이미 사용자 정보를 업데이트했으므로 사용자 정보를 다시 조회
        const userResponse = await fetch('/api/users');
        console.log('[Client] 사용자 정보 조회 응답 상태:', userResponse.status);
        
        if (userResponse.ok) {
          const updatedUser = await userResponse.json();
          console.log('[Client] 업데이트된 사용자 정보:', updatedUser);
          setUser(updatedUser);
          ...
          console.error('[Client] 사용자 정보 조회 실패:', errorData);
          throw new Error('사용자 정보 업데이트 확인에 실패했습니다.');
        }
      } else {
        console.error('[Client] 이미지 업로드 실패:', data);
