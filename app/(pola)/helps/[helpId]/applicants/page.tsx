...
export default function ApplicantsPage() {
  ...
  const { data: applicantsData, isLoading: isLoadingApplicants, error: applicantsError } = useHelpApplicants(helpId);
  const { mutate: acceptApplicant, isPending: isAccepting } = useAcceptHelpApplicant();
  ...
  if (applicantsError) {
    return <div className={styles.error}>지원자 목록을 불러오는데 실패했습니다.</div>;
  }
...
function ApplicantItem({ applicant, onAccept, isAccepting }: ApplicantItemProps) {
  const { data: userProfile, isLoading: isLoadingProfile } = useUserProfile(applicant.juniorNickname);

  if (isLoadingProfile) {
    return <div className={styles.loadingItem}>프로필 정보를 불러오는 중...</div>;
  }

  if (!userProfile) {
    return <div className={styles.errorItem}>프로필 정보를 불러올 수 없습니다.</div>;
  }
  ...
  const userProfileDto: UserProfileResponseDto = {
    ...
    userType: 'junior', // 지원자는 모두 junior
  };
  ...
  return (
    <div className={styles.applicantItem}>
      ...
      <div className={styles.applicantInfo}>
        <div className={`${styles.status} ${applicant.isAccepted ? styles.accepted : styles.pending}`}>
        ...
        <button 
          className={`${styles.acceptButton} ${applicant.isAccepted ? styles.acceptedButton : ''}`}
