...
export async function POST(
  ...
  console.log('[API] POST /api/images/review 호출됨');

  try {
    // 사용자 인증 - 쿠키에서 nickname 추출
    const nickname = "jelly5361"; //getNicknameFromCookie(request);
    if (!nickname) {
    ...
    // 여러 가능한 파일 필드명을 시도
    const possibleFileKeys = ['file', 'image', 'upload', 'photo', 'reviewImage'];
    ...
    console.log(
      `[API] 리뷰 이미지 업로드 시작 - 사용자: ${nickname}`
    );
    ...
    console.log('[API] 리뷰 이미지 업로드 성공:', result.url);
